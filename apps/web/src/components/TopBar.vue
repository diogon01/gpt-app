...
const showLogin = ref(false); // modal control

const userStatus = computed(() => {
  if (!auth.user) return 'anonymous';
  return auth.user.isPlus ? 'plus' : 'free';
});
...
<template>
  <header class="relative h-14 flex items-center border-b border-slate-700 bg-slate-950 px-4">
    ...
    <div class="absolute left-1/2 -translate-x-1/2">
      <select
        v-model="ui.model"
        class="bg-slate-800 border border-slate-700 text-cyan-400
               rounded px-3 py-1 text-sm focus:outline-none"
      >
        <option
          v-for="m in models"
          :key="m.value"
